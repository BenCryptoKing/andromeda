...
#if defined(NDEBUG)
# error "Andromeda cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8e89c7c0209b715bbbbee45c2e25190e418a4178f1b1aead142d2fec33fcbf37");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Andromeda: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Andromeda Signed Message:\n";
...
{
    // Andromeda: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Andromeda: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Andromeda: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Andromeda: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Andromeda: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Andromeda: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb5eb3fb63702b6027fbf99443c624f75bd18db172d4680d3a5d0a2c6ddae01bd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Luanda, ANG 28/Jan/2020 - BEN's Xtreme Profission";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f3452d521fe872afdc515cfb5bd69192a62995aba49f8708b85b249b99574ba85c8379c3ad6bbef9343088e44ac1d09a91085307a04b20df4a435dbfb5afa7a8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1580208245;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086603415;

        if (fTestNet)
        {
            block.nTime    = 1580208207;
            block.nNonce   = 387385242;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x8a4a40017c481b54cdd3516f1f494586f4b6410a34bc8942b0b6625a801da899"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xdc }; // Andromeda: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AndromedaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AndromedaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AndromedaMiner(CWallet *pwallet)
{
    printf("AndromedaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("andromeda-miner");
    ...
    try { loop {
        ...
        printf("Running AndromedaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AndromedaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AndromedaMiner, pwallet));
}
